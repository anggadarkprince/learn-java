...
<!-- if the data is not set, redirect to controller to get country data -->
<c:choose>
    ...
    <tr>
        ...
        <td style="width:75%;height:80%;">
            ...
            <c:forEach var="tempCountry" items="${sessionScope.countryData}">
                <c:if test="${tempCountry.population lt 100000}">
                    <h3>*****************************************************</h3>
                    <h3>Country: ${tempCountry.code} has population = ${tempCountry.population}</h3>
                    <c:set var="updateCountry" value="${tempCountry}"/>
                    <h3>Before changes: ${updateCountry}</h3>
                    <c:set target="${updateCountry}" property="population" value="200000"/>
                    <h3>Please update ${tempCountry.name} to gain pop ==> ${updateCountry.population}</h3>
