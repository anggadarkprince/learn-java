package com.sketchproject.worldmanager.models;
...
public class CountryTest {
    ...
    public void testMutatorsAndAccessors() {
        ...
        assertEquals("Could not set indep year as expected", indepYear, c.getIndepYear());
        //population = 350000000;
        c.setPopulation(population);
        assertEquals("Could not set population as expected", population, c.getPopulation());
        //lifeExpectancy = 73;
        c.setLifeExpectancy(lifeExpectancy);
        assertEquals("Could not set life as expected", lifeExpectancy, c.getLifeExpectancy(), delta);
        //gnp = 15000000000000.0;
        c.setGnp(gnp);
        assertEquals("Could not set gnp as expected", gnp, c.getGnp(), delta);
        //gnpOld = 14000000000000.0;
        c.setGnpOld(gnpOld);
        assertEquals("Could not set gnp old as expected", gnpOld, c.getGnpOld(), delta);
        //localName = "USA";
        c.setLocalName(localName);
        assertEquals("Could not set local name as expected", localName, c.getLocalName());
        //governmentForm = "Democracy";
        c.setGovernmentForm(governmentForm);
        assertEquals("Could not set government as expected", governmentForm, c.getGovernmentForm());
        //headOfState = "The Current President";
        c.setHeadOfState(headOfState);
        assertEquals("Could not set head of state as expected", headOfState, c.getHeadOfState());
